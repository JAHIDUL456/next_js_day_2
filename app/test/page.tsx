...
export default function Home() {
  ...
  const del = async (id: number) => {
    try {
      await axios.delete(`https://api.fake-rest.refine.dev/${type}/${id}`);
  ...
  const views = (title: string, content: string) => {
    setModalTitle(title);
  ...
  const edit = (item: BlogPost | Category) => {
    setEditId(item.id);
    setModalTitle(item.title);
    setModalContent((item as BlogPost).content || '');
    setIsEditMode(true);
    setShowModal(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.patch(`https://api.fake-rest.refine.dev/${type}/${editId}`, {
        title: modalTitle,
        ...(type === 'blog_posts' && { content: modalContent }),
      });

      setData(data.map(item =>
        item.id === editId
          ? { ...item, title: modalTitle, ...(type === 'blog_posts' && { content: modalContent }) }
          : item
      ));
                  ...
                  {currentData.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td className='px-4 py-2 border-2 flex gap-2 justify-center'>
                        <button onClick={() =>
                          type === 'blog_posts'
                            ? views((item as BlogPost).title, (item as BlogPost).content)
                            : views(item.title, '')
                        }>
                          <GrView />
                        </button>
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white text-black p-6 rounded-md w-[90%] max-w-md relative">
            <h2 className="text-2xl font-bold mb-4">
              {isEditMode ? "Edit Item" : modalTitle}
            </h2>
            ...
            {isEditMode ? (
              ...
              <>
                {modalContent && <p className="mb-6">{modalContent}</p>}
              </>
            )}
